...
class CartDAO
{
    ...
    public function get_cart_by_memberid(int $memberid)
    {
        ...
        FROM Cart INNER JOIN Goods ON Cart.goodscode = Goods.goodscode WHERE memberid = :memberid";
    ...
    public function get_cart_num(int $memberid)
    {
        $dbh = DAO::get_db_connect();

        $sql = "SELECT SUM(num) AS 'sum' FROM Cart WHERE memberid = :memberid";
        $stmt = $dbh->prepare($sql);
